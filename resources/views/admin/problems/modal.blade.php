<div class="modal fade" id="problem-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="problem-form" method="POST" class="form-horizontal">
                {{ csrf_field() }}
...
@push('scripts')
    <script>
        ...
        $(document).on('show.bs.modal','#problem-modal', function (event) {  // shown.bs.modal
            ...
            if( id ){
                ...
                $('#_method').val("PUT");
                $('#problem-form').attr('action', url);
                $('.modal-title').text("Actualizar Problema "+name)
                $('#button-submit').text("Actualizar");
            } else {
                document.getElementById("name").value = "";
                $('#problem-form').attr('action', url);
                $('.modal-title').text("Crear nuevo Problema")
